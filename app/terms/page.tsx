      ...
      <nav className="fixed top-0 w-full bg-black/80 backdrop-blur-md z-50 border-b border-zinc-800">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            <Link href={basePath ? `${basePath}/terms` : "/terms"} className="text-sm font-medium text-white transition-colors">{t.nav.terms}</Link>
            <Link href={basePath ? `${basePath}/privacy` : "/privacy"} className="text-sm font-medium text-zinc-300 hover:text-white transition-colors">{t.nav.privacy}</Link>
            <LocaleSwitcher />
          </div>
        </div>
      </nav>
